...
namespace IDS.Lexik.OWIDplusLIVE.Upload
{
  class Program
  {
    ...
    private static void SendData(string url, string token, DateTime date, string path, string secret)
    {
      ...
      var client = new RestClient($"{url}update") {Timeout = -1};
      var request = new RestRequest(Method.POST);
      request.AddParameter("application/json", JsonConvert.SerializeObject(new CowidPlusUpdateRequest
      {
        Data = data,
        Day = date.Day,
        Month = date.Month,
      ...
      Console.WriteLine(client.Execute(request).StatusCode);
    }

    private static string GetToken(string url)
    {
      var client = new RestClient($"{url}token") { Timeout = -1 };
