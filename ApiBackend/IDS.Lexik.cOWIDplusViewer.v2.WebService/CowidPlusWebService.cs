...
namespace IDS.Lexik.cOWIDplusViewer.v2.WebService
{
  public class CowidPlusWebService : AbstractEasyWebService<CowidPlusWebConfiguration>
  {
    ...
    {
      server.AddEndpoint(HttpVerb.GET, "/init", Init);
      server.AddEndpoint(HttpVerb.POST, "/find", Find);
      server.AddEndpoint(HttpVerb.POST, "/pull", Pull);
      server.AddEndpoint(HttpVerb.GET, "/norm", Norm);
      server.AddEndpoint(HttpVerb.POST, "/down", Down);

      server.AddEndpoint(HttpVerb.GET, "/token", Token);
      server.AddEndpoint(HttpVerb.POST, "/update", Update);
    }

    private Task Token(HttpContext arg)
    {
      try
      {
        _lastUpdateToken = Guid.NewGuid().ToString("N");
        return arg.Response.Send(_lastUpdateToken);
      }
      catch (Exception ex)
      {
